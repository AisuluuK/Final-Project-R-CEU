...
```{r, echo=FALSE}
summary_dtf <- summary(dtflights2)
...
minavdelay <- dtflights2[, .(AveDelay = mean(arr_delay, na.rm = TRUE)), by = name]
...
```{r, echo=FALSE}
top_delays <- minavdelay[minavdelay$AveDelay >= minavdelay$AveDelay[head(order(minavdelay$AveDelay, decreasing=TRUE))][5] , ]
top_delays$name
```
...
```{r, echo=FALSE}
low_delays <- minavdelay[minavdelay$AveDelay <= minavdelay$AveDelay[head(order(minavdelay$AveDelay, decreasing=FALSE))][5] , ]
low_delays$name
```
...
```{r, echo=FALSE}
ggplot(minavdelay, aes(x=AveDelay)) +
...
```{r, echo=FALSE}
plot(minavdelay$name, minavdelay$AveDelay,  col="blue", lwd = 5, xlab="Airlines", ylab="Arrival Delay", main="Average of delays")
```
...
##Modeling - part 2 - with binary outcome

Let's create a binary variable within dataset, that would take value of 1 if flight was late by more than 15 minutes at arrival, and 0 otherwise. We will try cart model to predict if the flight will be late by more than 15 minutes, based on the company that served a flight, distance of the flight  average daily temperature and visibility of the flight date. For this we need to use *weather* dataset.
```{r, echo=FALSE}
...
setkey(dtflights3, year, month, day)
dtflights5 <- dtflights3[daily_visib]
daily_temp <- weather[, .(ave_temp = mean(temp)), by = .(year, month, day)]
setkey(daily_temp, year, month, day)
setkey(dtflights5, year, month, day)
dtflights5 <- dtflights5[daily_temp]

dtflights5 <- data.frame(dtflights5)
dtflights5["delayed"] <- ifelse(dtflights5$arr_delay >= 15, 1, 0)
dtflights5 <- data.table(dtflights5)


```

We tested using clasification tree technique (2 methods) to see the probability of the flight being late by more than 15 minutes. "Class" method, which is tree alike classification and "Anova" method of regression. It was not clear how to estimate the results, since the tree has only two possible outcomes.(the code chunks are available) I would prefer to go with linear model with the same control varibles. Let's try it:

```{r, echo=FALSE}
#rp1 <- rpart(delayed ??? name + distance + ave_visib + ave_temp, data = dtflights5, method="class")
#pander(printcp(rp1))
```
  

```{r, echo=FALSE}
#rp2 <- rpart(delayed ??? name + distance + ave_visib + ave_temp, data = dtflights5, method="anova")
#pander(printcp(rp2))
```

```{r, echo=FALSE}
lm1 <- lm(delayed ~ distance + ave_visib + ave_temp + name, data = dtflights5)
pander(summary(lm1))
```

From the regression results, we see that increase in distance is associated with the higher probability of flight being late by more than 15 minutes. increase in average visibility tends to decrease the probaility of 15min delay, average temperature has insignificant results. All of the airline companies tend to have negative correlation with delays, except for Frontier Airlines Inc. However, 
